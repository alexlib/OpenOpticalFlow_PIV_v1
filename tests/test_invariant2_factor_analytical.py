...
# Helper function to run test for a specific flow pattern
def run_invariant2_test(name, vortex_func, X, Y):
    print(f"\nTesting {name}:")
    ...
    for impl_name, q in results.items():
        ...
        if np.any(nonzero):
            ...
            scale_factors[impl_name] = scale_factor
            ...
            errors[impl_name] = (mean_abs_error, max_abs_error)
    ...
    for i, (impl_name, q) in enumerate(results.items()):
        if i < len(positions):
            ...
            nonzero = np.abs(q_analytical) > 1e-10
            if np.any(nonzero):
                scale_factor = scale_factors[impl_name]
                q_scaled = q * scale_factor
            else:
                q_scaled = q
                scale_factor = 1.0
...
def test_taylor_green_vortex():
    X, Y, x, y, n = create_grid()
    results, scale_factors, errors = run_invariant2_test("Taylor-Green Vortex", lambda: taylor_green_vortex(X, Y), X, Y)
...
def test_rankine_vortex():
    X, Y, x, y, n = create_grid()
    results, scale_factors, errors = run_invariant2_test("Rankine Vortex", lambda: rankine_vortex(X, Y), X, Y)
...
# Test for Stagnation Point flow
def test_stagnation_point_flow():
    X, Y, x, y, n = create_grid()
    results, scale_factors, errors = run_invariant2_test("Stagnation Point Flow", lambda: stagnation_point_flow(X, Y), X, Y)

...
def test_flow_analysis():
    ...
    for name, vx, vy in test_cases:
        ...
        print(f"  Q-criterion: open={q_open_mean:.4f}, comparison={q_comp_mean:.4f}, corrected={q_corrected_mean:.4f}")
        ...
        # Differences
        plt.subplot(337)
        diff = np.abs(vort_open - vort_comp)
        plt.imshow(diff, cmap='viridis')
        plt.colorbar()
        plt.title('Vorticity Difference')
        
        plt.subplot(338)
        diff = np.abs(q_open - q_comp)
        plt.imshow(diff, cmap='viridis')
        plt.colorbar()
        plt.title('Q-criterion Difference')
        
        plt.subplot(339)
        diff = np.abs(q_open - q_corrected)
        plt.imshow(diff, cmap='viridis')
        plt.colorbar()
        plt.title('Q vs Corrected Difference')
